...
Определите переменные окружения `SECRET_KEY` и `YANDEX_GEOCODER_API_KEY`. Создайте файл `.env` в каталоге `star_burger/` и положите туда такой код:
```sh
SECRET_KEY=django-insecure-0if40nf4nf93n4
YANDEX_GEOCODER_API_KEY=ваш_ключ_от_яндекса
```

Для преобразования адресов клиентов в координаты (широту и долготу) используется сервис Яндекс Геокодер. Это нужно, чтобы автоматически определять ближайший ресторан, который сможет выполнить доставку.

Чтобы использовать его, необходимо получить **API-ключ** и записать его в файл `.env` в переменную `YANDEX_GEOCODER_API_KEY`.
...
- `YANDEX_GEOCODER_API_KEY`=ваш ключ от сервиса Яндекса-геокодер
- `DATABASE_URL` — postgres://USER:PASSWORD@HOST:PORT/NAME
- `ROLLBAR_ACCESS_TOKEN` — ваш токен Rollbar

### Деплой:

Для запуска деплойного скрипта выполните:
...
При запуске скрипта:
1. Автоматически обновляется код на сервере.
2. Устанавливаются необходимые библиотеки.
3. Собирается статика.
4. Применяются миграции.
4. Перезапускаются сервисы (Nginx + Gunicorn).
5. Отправляется уведомление в Rollbar.
